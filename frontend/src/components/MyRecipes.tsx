      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 z-10 overflow-hidden">
        <div className="p-6">
          ...
          <p className="text-gray-600 text-center mb-6">
            You want to delete a recipe <span className="font-semibold">{recipeName}</span> Or not? This operation cannot be restored.
          </p>
...
export default function MyRecipes({ recipes, isAdminLoggedIn }: MyRecipesProps) {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const updatedRecipes = recipesUser.filter(recipe => recipe.recipe_id !== deleteConfirmation.recipeId);
      setRecipesUser(updatedRecipes);

      // Remove rating of deleted recipe
      setAverageRatings(prev => {
        const updated = { ...prev };
        delete updated[deleteConfirmation.recipeId];
        return updated;
      });
